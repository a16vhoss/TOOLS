...
export default function ChatDetailPage({ params }: { params: Promise<{ chatId: string }> }) {
  ...
  const currentUserId = conversation?.participants[0] || 'u1';
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <div className="flex items-center gap-3 px-4 h-14 border-b border-border bg-bg-white shrink-0">
        <Link href="/chat" className="p-1 no-underline"><ArrowLeft className="h-5 w-5 text-text" /></Link>
        <Avatar name={conversation.participantNames[1]} size="sm" />
        <div>
          <p className="text-sm font-semibold text-text">{conversation.participantNames[1]}</p>
          ...
          if (isSystem) {
            return (
              <div key={msg.id} className="flex justify-center">
                <div className="bg-info-light text-info text-xs px-3 py-1.5 rounded-full max-w-[80%] text-center">{msg.content}</div>
              </div>
            );
          }
          return (
      ...
      <div className="px-4 py-3 border-t border-border bg-bg-white shrink-0">
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 h-10 px-4 rounded-full border border-border bg-bg text-sm text-text placeholder:text-text-light focus:outline-none focus:ring-2 focus:ring-primary"
          />
          <button className="h-10 w-10 bg-primary rounded-full flex items-center justify-center shrink-0">
